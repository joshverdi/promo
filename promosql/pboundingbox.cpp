...
void PBoundingBox::compute_eigens(CGLA::Mat3x3f _input, CGLA::Mat3x3f& _eigenvec, CGLA::Vec3f& _eigenval)
{
    ...
    eigen_decomposition(mat_A, mat_V, d);
    for (int i=0; i<MATRIX_DIM; i++)
    {
        _eigenval[i]=d[i];
        for (int j=0; j<MATRIX_DIM; j++)
...
CGLA::Vec3f PBoundingBox::change_basis(CGLA::Vec3f& _point3d, CGLA::Vec3f& _translat, CGLA::Mat3x3f& _matrix, bool _trans_then_rotat)
{
    ...
    {
        value=translate(_point3d, _translat);
        value=_matrix*value;
    }
    else
    {
...
float PBoundingBox::volume()
{
    CGLA::Vec3f vec_length( vec_corner[1][0]-vec_corner[0][0],
                            vec_corner[1][1]-vec_corner[0][1],
                            vec_corner[1][2]-vec_corner[0][2]);
    CGLA::Vec3f vec_width(  vec_corner[2][0]-vec_corner[1][0],
                            vec_corner[2][1]-vec_corner[1][1],
                            vec_corner[2][2]-vec_corner[1][2]);
    CGLA::Vec3f vec_height( vec_corner[0][0]-vec_corner[4][0],
                            vec_corner[0][1]-vec_corner[4][1],
                            vec_corner[0][2]-vec_corner[4][2]);
    return vec_length.length()*vec_width.length()*vec_height.length();
}

CGLA::Vec3f PBoundingBox::center()
{
    CGLA::Vec3f value;
    std::vector< std::vector<float> > vec_corner_tmp;
    std::vector<float> vec_tmp, vec_center_tmp;
    for (std::vector<CGLA::Vec3f>::iterator iter = vec_corner.begin(); iter != vec_corner.end(); iter++)
    {
        vec_tmp.push_back((*iter)[0]);
        vec_tmp.push_back((*iter)[1]);
        vec_tmp.push_back((*iter)[2]);
        vec_corner_tmp.push_back(vec_tmp);
        vec_tmp.clear();
    }
    vec_center_tmp = PUtils::mean(vec_corner_tmp);
    if (vec_center_tmp.size()>=3)
        value = CGLA::Vec3f(vec_center_tmp[0], vec_center_tmp[1], vec_center_tmp[2]);
    return value;
}

CGLA::Vec3f PBoundingBox::u()
{
    CGLA::Vec3f value;
    value = CGLA::Vec3f (   (vec_corner[1][0]-vec_corner[0][0]),
                            (vec_corner[1][1]-vec_corner[0][1]),
                            (vec_corner[1][2]-vec_corner[0][2]) );
    value.normalize();
    value.operator *=(-1);
...
CGLA::Vec3f PBoundingBox::v()
{
    CGLA::Vec3f value;
    value = CGLA::Vec3f (   (vec_corner[2][0]-vec_corner[1][0]),
                            (vec_corner[2][1]-vec_corner[1][1]),
                            (vec_corner[2][2]-vec_corner[1][2]));
    value.normalize();
    value.operator *=(-1);
...
CGLA::Vec3f PBoundingBox::w()
{
    CGLA::Vec3f value;
    value = CGLA::Vec3f (   (vec_corner[0][0]-vec_corner[4][0]),
                            (vec_corner[0][1]-vec_corner[4][1]),
                            (vec_corner[0][2]-vec_corner[4][2]) );
    value.normalize();
    value.operator *=(-1);
...
float PBoundingBox::length()
{
    return CGLA::Vec3f (    vec_corner[1][0]-vec_corner[0][0],
                            vec_corner[1][1]-vec_corner[0][1],
                            vec_corner[1][2]-vec_corner[0][2]).length();
}

float PBoundingBox::width()
{
    return CGLA::Vec3f (    vec_corner[2][0]-vec_corner[1][0],
                            vec_corner[2][1]-vec_corner[1][1],
                            vec_corner[2][2]-vec_corner[1][2]).length();
}

float PBoundingBox::height()
{
    return CGLA::Vec3f (    vec_corner[0][0]-vec_corner[4][0],
                            vec_corner[0][1]-vec_corner[4][1],
                            vec_corner[0][2]-vec_corner[4][2]).length();
}
