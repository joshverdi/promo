...
namespace CGLA {
	...
	*/
	...
	public:
		...
		explicit Vec3f(const Vec3i& v): ArithVec<float,Vec3f,3>((float) v[0], (float) v[1], (float) v[2]) {}
	
		/// Construct from a 3D unsigned int vector.
		explicit Vec3f(const Vec3usi& v): ArithVec<float,Vec3f,3>((float) v[0],(float) v[1],(float) v[2]) {}
