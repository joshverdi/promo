...
namespace CGLA {
	...
	namespace TableTrigonometry
	{
		...
		const float ANGLE_FACTOR = (float) (ANGLE_MAX) / float(M_PI_2);
		...
		inline float angle2float(Angle theta)
		{
			...
			switch(theta & 3)
				...
				case 1: return (float) (M_PI - (theta>>2)/ANGLE_FACTOR);
				case 2: return (float) (M_PI + (theta>>2)/ANGLE_FACTOR);
				case 3: return (float) (MPI2 - (theta>>2)/ANGLE_FACTOR);
				}
			return 0;	
		}
